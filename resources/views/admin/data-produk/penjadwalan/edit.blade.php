                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="id_pesanan" class="form-select" required>
                                ...
                                @foreach ($pesanan as $item)
                                    <option value="{{ $item->id_pesanan }}" {{ $item->id_pesanan == $jadwal->id_pesanan ? 'selected' : '' }}>
                                        {{ $item->kode_pesanan }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status_jadwal" class="form-select" required>
                                ...
                                <option value="menunggu" {{ $jadwal->status_jadwal == 'menunggu' ? 'selected' : '' }}>Menunggu</option>
                                <option value="berjalan" {{ $jadwal->status_jadwal == 'berjalan' ? 'selected' : '' }}>Diproses</option>
                        ...
                        @foreach ($jadwal->users as $index => $petugas)
                            <div class="row mb-3 petugas-item">
                                <div class="col-md-6">
                                    ...
                                    <select name="user_id[]" class="form-select" required>
                                        ...
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}" {{ $user->id == $petugas->id ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="upah[]" class="form-control" min="0" value="{{ $petugas->pivot->upah }}" required>
                                </div>
