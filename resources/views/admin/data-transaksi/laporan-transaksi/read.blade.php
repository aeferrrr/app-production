        ...
        <div class="p-3">
            <form method="GET" action="{{ route('admin.transaksi.item-laporan') }}" class="row g-2 align-items-end">
                ...
                <div class="col-md-6 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.transaksi.export.excel', request()->only('tanggal_mulai', 'tanggal_selesai')) }}" class="btn btn-sm btn-success">
                        <i class="bx bx-file"></i> Export Excel
                    </a>
                    <a href="{{ route('admin.transaksi.export.pdf', request()->only('tanggal_mulai', 'tanggal_selesai')) }}" class="btn btn-sm btn-danger" target="_blank">
                            ...
                            <td>
                                <ul class="list-unstyled mb-0">
                                    @foreach($item->users as $user)
                                        <li>- {{ $user->name }} <span class="badge bg-primary ms-1">Rp{{ number_format($user->pivot->upah) }}</span></li>
                                    @endforeach
                                </ul>
                            </td>
                            <td>
